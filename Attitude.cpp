...
{
    float h_Th[2];
    int32_t z_cm = current_loc.alt;		// 現在のPOS高度 [m]: POS推定の絶対高度は誤差が大きい
    int32_t z_r_cm = next_WP_loc.alt;	// 目標高度 [m]: 次のWPの高度(相対高度で指定する必要あり)
    z = z_cm * 0.01f;		// 単位変換 [cm] -> [m]
    z_r = z_r_cm * 0.01f;	// 単位変換 [cm] -> [m]
    uint64_t current_time_Th = AP_HAL::micros64();	// 現在の時刻 [us](uint)
    ...
    past_time_Th = current_time_Th - prev_time_Th;	// 1ループ前からの経過時間 [us](uint)
    float past_time_Th_f = (float)past_time_Th;		// データ型変換 (uint) -> (float)
    past_time_Th_f *= 1e-6;		// 単位変換 [us] -> [s]

    // エラーがあった場合(高度変化なし or 経過時間が0 [us])
    ...
    else
    {
        ...
        z_old = z_cm;	// 直前の高度 [cm]の更新
        prev_time_Th = current_time_Th;	// 直前の時刻 [us]の更新
    }

    // エラーがあった場合(ピッチ角変化なし or 経過時間が0 [us])
    ...
    {
        d_pitch = ahrs.pitch - pitch_old;	// ピッチ角の変化量 [rad]
        //float dz_f = (float)dz_int;
        speed_pitch = d_pitch/(past_time_Th_f);	// ピッチ角速度 [rad/s]
        pitch_old = ahrs.pitch;	// 直前のピッチ角 [rad]を更新
        prev_time_Th = current_time_Th;	// 直前の時刻 [us]の更新
    }
    ...
    // 風洞実験の式を使用して 推力の平衡点 [N]の計算(2.49 [N] MPで入力)
