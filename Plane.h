...
// Inertial Navigation EKF
    ...
    */
    ...
    void Log_Write_PPG3();
    void Log_Write_PPG4();
    void Log_Write_PPG5();//add by aoki
    void Log_Write_PPG6();//added by hatae
    ...
    void avoid_adsb_run();
    float calc_controller(float x1, float x2);
    int32_t TLAB_Throttle_Controller(void);
    float thrust_to_percent(float);
    int32_t TLAB_Line_Trace_Controller(void); // added by iwase 17/06/26
    void init_TLAB_Controller(void);
    void init_TLAB_Controller_AUTO(void);
    int32_t TLAB_Circle_Trace_Controller(void);
    int32_t TLAB_Combine_Controller(void);
//    int32_t TLAB_Fuzzy_Line_Trace_Controller(void);
//    int32_t TLAB_Fuzzy_Circle_Trace_Controller(void);
    ...
    bool firsttime_Th;
    bool use_fuzzy_controller;
    int8_t TLAB_Control_flag;
    float v_a;
    float v_g;
    float Vg_limited;
    float u;
    ...
    float U_max;
    int32_t servo;
    uint16_t TLAB_CMD_index;
    bool TLAB_WP_nav_flag;
    ...
    float motor_per;
    float motor_neutral;
    uint64_t prev_time_Th;
    uint32_t past_time_Th;
    float kp_Th[2];
    float kd_Th[2];
    float d1_Th;
    float d2_Th;
    ...
    float F3;
    float F4;
    float gps_dh;//add by aoki 20210324
    ...
    float state_UAV_y;
    float state_UAV_phi;
    float state_UAV_GCRS;
    float Dist_currWP2UAV;
    float rad_prevWP2UAV;
    float rad_prevWP2currWP;
    float a_bottom;
    float a_above;
    float det_a;
    float det_b;
    ...
    float e_r;
    float e_chi;
    float prev_theta; // 積分用
    float diff_theta; //thetaの差分
    float Int_theta; // thetaの積分
    Location prev_POS;
    Location mid_POS;
    ...
    float xI;  // 慣性座標 x (緯度方向) [m]
    float yI;  // 慣性座標 y (経度方向) [m]
